...
SELECT * FROM hr_analysis.`hr data.xlsx - hr data`;

-- Check missing values
SELECT COUNT(*) FROM hr_analysis.`hr data.xlsx - hr data`
...
`Years With Curr Manager` IS NULL ;

-- Remove unwanted columns and changing table name 
ALTER TABLE hr_analysis.`hr data.xlsx - hr data` DROP COLUMN `CF_age band`, DROP COLUMN `CF_attrition label` , DROP COLUMN `CF_current Employee`, DROP COLUMN `Employee Count`,DROP COLUMN Over18, DROP COLUMN `Standard Hours`, DROP COLUMN `-2`, DROP COLUMN `0`,DROP COLUMN `emp no`;
SELECT * FROM hr_analysis.`hr data.xlsx - hr data`;
RENAME TABLE hr_analysis.`hr data.xlsx - hr data` TO HR_employee ; 
